...
var FormControl = React.createClass({
  ...
  render: function () {
    ...
    if (attr.mappedBy && attr.fieldType === 'ONE_TO_MANY') {
      var mappedBy = attr.refEntity.attributes[attr.mappedBy]
      if (!mappedBy || mappedBy.nillable) {
        // See #6608: the mappedBy attribute is not necessarily available here!
        attributeControlProps.readOnly = true
      }
    }
